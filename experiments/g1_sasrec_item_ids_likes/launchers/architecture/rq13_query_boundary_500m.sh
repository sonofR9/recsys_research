...
if [[ "$#" -lt 1 ]]; then
    echo "usage: $0 RQ13_BOUNDARY_RUN [...]" >&2
    exit 2
fi
...
from experiments.g1_sasrec_item_ids_likes.analysis.rq13_rq14_query_candidates import validated_required_boundary_candidates

document = json.loads(Path(sys.argv[1]).read_text())
candidates = validated_required_boundary_candidates(document, sys.argv[2:])
for candidate in candidates:
    print(f"{candidate.run_name}\t{candidate.treatment}")
PY
)
if [[ "${#candidate_rows[@]}" -ne "$#" ]]; then
    echo "Requested runs are not the exact current RQ13 boundary followups" >&2
    exit 2
fi
...
echo "=== rq13 boundary native-500M: enqueued=${enqueued_count}, skipped=${skipped_count} ==="
