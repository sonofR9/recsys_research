...
if [[ "$#" -eq 1 && "$1" == "--list" ]]; then
    python - <<'PY'
from experiments.g1_sasrec_item_ids_likes.analysis.rq13_rq14_query_candidates import rq14_initial_candidates
...
if [[ "${#candidate_rows[@]}" -ne 12 ]]; then
    echo "RQ14 initial stage must contain exactly 12 runs" >&2
    exit 2
fi
...
for row in "${candidate_rows[@]}"; do
    ...
    if [[ -z "$run" || -n "${seen[$run]+x}" ]]; then
        echo "Invalid or duplicate RQ14 row: $row" >&2
        exit 2
    fi
    seen[$run]=1
...
echo "=== rq14 native-500M: enqueued=${enqueued_count}, skipped=${skipped_count} ==="
