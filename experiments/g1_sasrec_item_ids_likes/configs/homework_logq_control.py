...
run = os.environ["G1_HOMEWORK_LOGQ_RUN"]
if re.fullmatch(r"[a-z0-9_]+", run) is None:
    raise ValueError("G1_HOMEWORK_LOGQ_RUN must contain lowercase letters, digits, and _")
epochs = _positive_integer("G1_HOMEWORK_LOGQ_EPOCHS")
if epochs < 20:
    raise ValueError("G1_HOMEWORK_LOGQ_EPOCHS must be at least 20")
revision = _positive_integer("G1_HOMEWORK_LOGQ_RUN_REVISION")
suffix = f"_ts{GENERATION_TRAINING_SEMANTICS_REVISION}_r{revision}"
if epochs != 20:
    suffix = f"_cap{epochs}{suffix}"
if not run.endswith(suffix):
    raise ValueError(f"G1_HOMEWORK_LOGQ_RUN must end with {suffix}")

dataset_size = os.environ.get("G1_HOMEWORK_LOGQ_DATASET_SIZE", "50m")
if dataset_size not in {"50m", "500m"}:
    raise ValueError("G1_HOMEWORK_LOGQ_DATASET_SIZE must be 50m or 500m")
embedding_lr = _positive_float("G1_HOMEWORK_LOGQ_EMBEDDING_LR")
deep_lr = _positive_float("G1_HOMEWORK_LOGQ_DEEP_LR")
environment_names = (
    "G1_DATASET_SIZE",
    "G1_MAX_EPOCHS",
    "G1_VARIANT",
...
experiment = replace(
    control,
    run_name=f"g1_homework_logq_{run}_{dataset_size}",
