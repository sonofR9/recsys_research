...
The architecture control's SwiGLU intermediate width is 192 —
...
| width | where |
| ---: | --- |
| 192 | the shared FFN control for both RQ4 families, hardcoded in [`configs/variant.py`](../configs/variant.py) and [`manifest.sh`](../launchers/architecture/manifest.sh) |
| 43, 86, 342, 684 | the RQ8 model-dimension axis, ratio-scaled from 192 at dims 16, 32, 128, 256 |
...
In the projections alone the penalty is real and large: 192 is 50% slower than
...
| 192 | GELU | 11.74, 11.78, 11.81 |
| 256 | GELU | 11.79 |
| 224 | SwiGLU | 11.87 |
| 384 | GELU | 11.86 |

A 12× width range moves the epoch by under 2%, and misaligned 192 sits between
...
Not fixed. Changing 192 changes the architecture control, and every axis in
...
probe already does — 128, not 192 — and it is the best FFN point measured on
