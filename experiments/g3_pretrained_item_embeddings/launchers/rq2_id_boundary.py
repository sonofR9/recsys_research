...
JOB_ENVIRONMENT = "G3_RQ2_ID_BOUNDARY_JOB_B64"
LEDGER_ENVIRONMENT = "G3_RQ2_ID_BOUNDARY_LEDGER_PATH"


def compile_rq2_id_boundary_queue_commands(
    *,
    ledger_path: Path,
    ...
    return compile_queue_commands(
        ...
        state_dir=state_dir,
        batch_id=batch_id,
        runner_script=Path(__file__).with_name("run_rq2_id_boundary.py"),
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
        ledger_validator=validate_rq2_id_boundary_ledger_document,
    )


def submit_rq2_id_boundary_jobs(
    *, ledger_path: Path, state_dir: Path, dry_run: bool
) -> str:
    ledger_path = ledger_path.resolve()
    ledger = load_rq2_id_boundary_ledger(ledger_path)
    verify_rq2_id_boundary_inputs(PROJECT_ROOT, ledger, full_validation=True)
    if dry_run:
        return "\n".join(
            shlex.join(command)
            for command in compile_rq2_id_boundary_queue_commands(
                ledger_path=ledger_path,
                ledger=ledger,
                state_dir=state_dir,
            )
    ...
    with ledger_submission_lock(
        ...
        return _submit_rq2_id_boundary_jobs(
            ledger_path=ledger_path,
            ledger=ledger,
            state_dir=state_dir,
        )
...
def _submit_rq2_id_boundary_jobs(
    *, ledger_path: Path, ledger: Rq2IdBoundaryLedger, state_dir: Path
) -> str:
    ...
    if existing is not None:
        return existing
    initial = compile_rq2_id_boundary_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
    )
...
def verify_rq2_id_boundary_inputs(
    root: Path,
    ledger: Rq2IdBoundaryLedger,
    *,
    ...
    root = root.resolve()
    evidence_path = _resolve_reference(root, ledger.next_stage_evidence.path)
    evidence = (
        verify_rq2_next_stage_evidence(evidence_path, root=root)
        if full_validation
        else load_rq2_next_stage_evidence(evidence_path)
    )
    if evidence["sha256"] != ledger.next_stage_evidence.sha256:
        raise ValueError("RQ2 ID boundary next-stage evidence hash changed")
    next_stage = load_rq2_next_stage_ledger(
        _resolve_reference(root, ledger.next_stage_ledger.path)
    )
    if next_stage.sha256 != ledger.next_stage_ledger.sha256:
        raise ValueError("RQ2 ID boundary next-stage ledger hash changed")
    evidence_ledger = evidence.get("next_stage_ledger")
    if (
        not isinstance(evidence_ledger, dict)
        or evidence_ledger.get("path") != ledger.next_stage_ledger.path
        or evidence_ledger.get("logical_sha256") != ledger.next_stage_ledger.sha256
    ):
        raise ValueError("RQ2 ID boundary evidence binds a different ledger")
    return verify_rq2_next_stage_inputs(
        root,
        next_stage,
...
def compiled_rq2_id_boundary_job_from_environment() -> tuple[
...
def build_training_experiment(
    ...
    *,
    ...
    if not all(isinstance(value, dict) for value in (representation, training, dataset)):
        raise ValueError("compiled G3 RQ2 ID boundary job is invalid")
    if (
        job.get("family_id") != "rq2_id_only_densenet"
        or job.get("phase") != "deep_learning_rate_lower_boundary_extension"
        or representation.get("id") != "rq2_id_only_densenet"
        or representation.get("history_hidden_dim") != 255
    ):
        raise ValueError("compiled G3 RQ2 ID boundary coordinate is invalid")
...
def write_job_contract(
    ...
    if verified != compiled:
        raise ValueError("compiled RQ2 ID boundary job differs from its ledger row")
    return persist_job_contract(
        compiled=compiled,
        ledger_path=ledger_path,
        ledger_sha256=ledger.sha256,
        logs_root=logs_root,
        filename="g3_rq2_id_boundary_job.json",
    )
...
def _resolve_reference(root: Path, value: str) -> Path:
    ...
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError("RQ2 ID boundary reference must be project-relative")
    path = root / relative
    if path.is_symlink() or not path.is_file() or not path.resolve().is_relative_to(root):
        raise ValueError(f"RQ2 ID boundary reference is not a project file: {value}")
    return path
