...
from experiments.g3_pretrained_item_embeddings.protocol.rq5_mechanism_ledger import (
    RQ5_MECHANISM_LEDGER_PATH,
    Rq5MechanismLedger,
    compile_rq5_mechanism_ledger,
    load_rq5_mechanism_ledger,
    persist_rq5_mechanism_ledger,
    verify_rq5_mechanism_inputs,
)


RQ5_MECHANISM_LEDGER_LOGICAL_SHA256 = (
    "b3f92d010dac9510a8263ae7f79d077ad65b72c7f6f422d6af408d36242f0fa5"
)
JOB_ENVIRONMENT = "G3_RQ5_MECHANISM_JOB_B64"
LEDGER_ENVIRONMENT = "G3_RQ5_MECHANISM_LEDGER_PATH"


def preview_rq5_mechanism_ledger(
    *, root: Path = PROJECT_ROOT
) -> Rq5MechanismLedger:
    ledger = compile_rq5_mechanism_ledger(root=root)
    if ledger.sha256 != RQ5_MECHANISM_LEDGER_LOGICAL_SHA256:
        raise ValueError("RQ5 mechanism preview differs from approved SHA")
    return ledger


def materialize_rq5_mechanism_ledger(
    path: Path | None = None, *, root: Path = PROJECT_ROOT
) -> Path:
    ledger = preview_rq5_mechanism_ledger(root=root)
    destination = root / RQ5_MECHANISM_LEDGER_PATH if path is None else path
    return persist_rq5_mechanism_ledger(destination, ledger, root=root)


def compile_rq5_mechanism_queue_commands(
    *,
    ledger_path: Path,
    ...
    if len(ledger.rows) != 2 or len(ledger.physical_rows) != 2:
        raise ValueError("RQ5 mechanism queue requires exactly two jobs")
    commands = compile_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
        runner_script=Path(__file__).with_name("run_rq5_mechanism.py"),
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
        ledger_validator=lambda document: _validate_document(document, ledger),
    )
    if len(commands) != 5:
        raise ValueError("RQ5 mechanism queue command count changed")
    return commands


def submit_rq5_mechanism_jobs(
    *,
    ledger_path: Path,
    state_dir: Path,
    ...
    expected = preview_rq5_mechanism_ledger(root=root)
    if expected.sha256 != expected_ledger_sha256:
        raise ValueError("RQ5 external mechanism SHA differs from preview")
    ledger_path = ledger_path.resolve(strict=True)
    ledger = load_rq5_mechanism_ledger(
        ledger_path, root=root, expected_ledger_sha256=expected.sha256
    )
    verify_rq5_mechanism_inputs(root, ledger)
    if dry_run:
        return "\n".join(
            shlex.join(command)
            for command in compile_rq5_mechanism_queue_commands(
...
def _submit(
    *,
    ...
    if existing is not None:
        return existing
    initial = compile_rq5_mechanism_queue_commands(
        ledger_path=ledger_path, ledger=ledger, state_dir=state_dir
    )
    ...
    commands = compile_rq5_mechanism_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
...
def compiled_rq5_mechanism_job_from_environment(
    *, root: Path = PROJECT_ROOT
) -> tuple[CompiledControlJob, Rq5MechanismLedger, Path, Path]:
    ...
    ledger = load_rq5_mechanism_ledger(
        ledger_path,
        root=root,
        expected_ledger_sha256=RQ5_MECHANISM_LEDGER_LOGICAL_SHA256,
    )
    compiled = decode_control_job(encoded, ledger)
    feature_path = verify_rq5_mechanism_inputs(root, ledger)
    return compiled, ledger, ledger_path, feature_path


def build_rq5_mechanism_training_experiment(
    ...
    *,
    ...
    if expected != compiled:
        raise ValueError("compiled RQ5 mechanism job differs from ledger")
    row = next((value for value in ledger.rows if value.id == compiled.row_id), None)
    if row is None:
        raise ValueError("RQ5 mechanism row is not launchable")
...
def write_rq5_mechanism_job_contract(
    compiled: CompiledControlJob,
    ledger: Rq5MechanismLedger,
    ledger_path: Path,
    logs_root: Path,
) -> Path:
    loaded = load_rq5_mechanism_ledger(
        ledger_path,
        root=PROJECT_ROOT,
    ...
    return persist_job_contract(
        ...
        logs_root=logs_root,
        filename="g3_rq5_mechanism_job.json",
    )


def _validate_document(
    document: dict[str, object], ledger: Rq5MechanismLedger
) -> None:
    if document != ledger.to_dict():
        raise ValueError("RQ5 mechanism document differs from approved ledger")
