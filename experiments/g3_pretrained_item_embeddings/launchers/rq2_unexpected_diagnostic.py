from __future__ import annotations

import argparse
import os
from pathlib import Path
import shlex
import subprocess

from experiments.g3_pretrained_item_embeddings.analysis.rq1_results import (
    verify_rq1_evidence,
)
from experiments.g3_pretrained_item_embeddings.analysis.rq2_content_deep_lr_boundary_results import (
    verify_rq2_content_deep_lr_boundary_evidence,
)
from experiments.g3_pretrained_item_embeddings.launchers.control import (
    PROJECT_ROOT,
    CompiledControlJob,
    compile_queue_commands,
    decode_control_job,
    encode_control_job,
    find_existing_ledger_batch,
    ledger_submission_lock,
    persist_job_contract,
)
from experiments.g3_pretrained_item_embeddings.launchers.rq2_content_deep_lr_boundary import (
    verify_rq2_content_deep_lr_boundary_inputs,
)
from experiments.g3_pretrained_item_embeddings.protocol.rq2_content_deep_lr_boundary_ledger import (
    RQ2_CONTENT_DEEP_LR_BOUNDARY_LEDGER_PATH,
    load_rq2_content_deep_lr_boundary_ledger,
)
from experiments.g3_pretrained_item_embeddings.protocol.rq2_unexpected_diagnostic_ledger import (
    APPROVED_RQ2_BOUNDARY_EVIDENCE_SHA256,
    APPROVED_RQ2_UNEXPECTED_DIAGNOSTIC_LEDGER_SHA256,
    Rq2UnexpectedDiagnosticLedger,
    load_rq2_unexpected_diagnostic_ledger,
    validate_rq2_unexpected_diagnostic_ledger_document,
)


JOB_ENVIRONMENT = "G3_RQ2_UNEXPECTED_DIAGNOSTIC_JOB_B64"
LEDGER_ENVIRONMENT = "G3_RQ2_UNEXPECTED_DIAGNOSTIC_LEDGER_PATH"


def compile_rq2_unexpected_diagnostic_queue_commands(
    *,
    ledger_path: Path,
    ledger: Rq2UnexpectedDiagnosticLedger,
    state_dir: Path,
    batch_id: str = "DRY_RUN_BATCH",
) -> list[list[str]]:
    if any(row.to_dict()["reused_from"] is not None for row in ledger.rows):
        raise ValueError("RQ2 diagnostic queue cannot enqueue reused cells")
    return compile_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
        runner_script=Path(__file__).with_name("run_rq2_unexpected_diagnostic.py"),
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
        ledger_validator=lambda document: validate_rq2_unexpected_diagnostic_ledger_document(
            document, root=PROJECT_ROOT
        ),
    )


def submit_rq2_unexpected_diagnostic_jobs(
    *, ledger_path: Path, state_dir: Path, dry_run: bool
) -> str:
    ledger_path = ledger_path.resolve()
    ledger = load_rq2_unexpected_diagnostic_ledger(ledger_path, root=PROJECT_ROOT)
    verify_rq2_unexpected_diagnostic_inputs(
        PROJECT_ROOT,
        ledger,
        full_validation=True,
    )
    if dry_run:
        return "\n".join(
            shlex.join(command)
            for command in compile_rq2_unexpected_diagnostic_queue_commands(
                ledger_path=ledger_path,
                ledger=ledger,
                state_dir=state_dir,
            )
        )
    with ledger_submission_lock(state_dir=state_dir, ledger_sha256=ledger.sha256):
        return _submit_rq2_unexpected_diagnostic_jobs(
            ledger_path=ledger_path,
            ledger=ledger,
            state_dir=state_dir,
        )


def _submit_rq2_unexpected_diagnostic_jobs(
    *,
    ledger_path: Path,
    ledger: Rq2UnexpectedDiagnosticLedger,
    state_dir: Path,
) -> str:
    runner_script = Path(__file__).with_name("run_rq2_unexpected_diagnostic.py")
    existing = find_existing_ledger_batch(
        state_dir=state_dir,
        ledger_path=ledger_path,
        ledger=ledger,
        runner_script=runner_script,
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
    )
    if existing is not None:
        return existing
    initial = compile_rq2_unexpected_diagnostic_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
    )
    subprocess.run(initial[0], cwd=PROJECT_ROOT, check=True, capture_output=True)
    created = subprocess.run(
        initial[1],
        cwd=PROJECT_ROOT,
        check=True,
        capture_output=True,
        text=True,
    )
    batch_id = created.stdout.strip()
    if not batch_id:
        raise RuntimeError("training queue returned an empty batch id")
    commands = compile_rq2_unexpected_diagnostic_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
    )
    for command in commands[2:]:
        subprocess.run(command, cwd=PROJECT_ROOT, check=True)
    return batch_id


def verify_rq2_unexpected_diagnostic_inputs(
    root: Path,
    ledger: Rq2UnexpectedDiagnosticLedger,
    *,
    full_validation: bool,
) -> Path:
    root = root.resolve(strict=True)
    if ledger.sha256 != APPROVED_RQ2_UNEXPECTED_DIAGNOSTIC_LEDGER_SHA256:
        raise ValueError("RQ2 unexpected diagnostic ledger is not approved")
    if full_validation:
        boundary = verify_rq2_content_deep_lr_boundary_evidence(
            root / ledger.boundary_evidence.path,
            root=root,
        )
        rq1 = verify_rq1_evidence(root / ledger.rq1_evidence.path, root=root)
        if (
            boundary.get("sha256") != ledger.boundary_evidence.logical_sha256
            or boundary.get("sha256") != APPROVED_RQ2_BOUNDARY_EVIDENCE_SHA256
            or rq1.get("sha256") != ledger.rq1_evidence.logical_sha256
        ):
            raise ValueError("RQ2 diagnostic fully verified evidence changed")
    boundary_ledger = load_rq2_content_deep_lr_boundary_ledger(
        root / RQ2_CONTENT_DEEP_LR_BOUNDARY_LEDGER_PATH,
        root=root,
    )
    return verify_rq2_content_deep_lr_boundary_inputs(
        root,
        boundary_ledger,
        full_validation=False,
    )


def compiled_rq2_unexpected_diagnostic_job_from_environment() -> tuple[
    CompiledControlJob,
    Rq2UnexpectedDiagnosticLedger,
    Path,
    Path,
]:
    encoded = os.environ.get(JOB_ENVIRONMENT)
    raw_ledger_path = os.environ.get(LEDGER_ENVIRONMENT)
    if not encoded or not raw_ledger_path:
        raise RuntimeError(f"{JOB_ENVIRONMENT} and {LEDGER_ENVIRONMENT} are required")
    ledger_path = Path(raw_ledger_path)
    ledger = load_rq2_unexpected_diagnostic_ledger(ledger_path, root=PROJECT_ROOT)
    feature_data_path = verify_rq2_unexpected_diagnostic_inputs(
        PROJECT_ROOT,
        ledger,
        full_validation=False,
    )
    return decode_control_job(encoded, ledger), ledger, ledger_path, feature_data_path


def build_training_experiment(
    compiled: CompiledControlJob,
    *,
    ledger: Rq2UnexpectedDiagnosticLedger,
    feature_data_path: Path,
):
    if ledger.sha256 != APPROVED_RQ2_UNEXPECTED_DIAGNOSTIC_LEDGER_SHA256:
        raise ValueError("RQ2 unexpected diagnostic ledger is not approved")
    expected = decode_control_job(encode_control_job(ledger, compiled.row_id), ledger)
    if expected != compiled:
        raise ValueError(
            "compiled RQ2 unexpected diagnostic job differs from its approved immutable ledger row"
        )
    from experiments.g3_pretrained_item_embeddings.configs.model import (
        G3Representation,
        build_g3_experiment,
    )

    job = compiled.job
    representation = job.get("representation")
    training = job.get("training")
    dataset = job.get("dataset")
    if not all(
        isinstance(value, dict) for value in (representation, training, dataset)
    ):
        raise ValueError("compiled G3 RQ2 unexpected diagnostic job is invalid")
    row_id = compiled.row_id
    expected_coordinates = {
        "rq2_unexpected_diagnostic:01": (
            "rq2_content_concat",
            "learned_item_id_plus_frozen_content",
            32,
            0.2183583071089141,
            0.021004505318001004,
        ),
        "rq2_unexpected_diagnostic:02": (
            "rq2_content_zero_id",
            "zero_frozen_item_id_plus_frozen_content",
            32,
            0.2183583071089141,
            0.021004505318001004,
        ),
        "rq2_unexpected_diagnostic:03": (
            "rq2_content_concat",
            "learned_item_id_plus_frozen_content",
            128,
            0.3041556165944196,
            0.014506684820055783,
        ),
    }
    family, history, hidden, embedding_lr, deep_lr = expected_coordinates[row_id]
    if (
        job.get("family_id") != family
        or job.get("phase") != "unexpected_result_diagnostic"
        or representation.get("id") != family
        or representation.get("history") != history
        or representation.get("catalog") != "learned_item_id"
        or representation.get("history_hidden_dim") != hidden
        or representation.get("content_trainable") is not False
        or representation.get("content_width") != 128
        or dataset.get("size") != "native-50m"
        or training.get("batch_size") != 512
        or training.get("seed") != 42
        or training.get("embedding_learning_rate") != embedding_lr
        or training.get("deep_learning_rate") != deep_lr
        or training.get("horizon_epochs") != 40
    ):
        raise ValueError("compiled G3 RQ2 unexpected diagnostic coordinate is invalid")
    return build_g3_experiment(
        run_name=str(job["run_name"]),
        dataset_size="native-50m",
        embedding_learning_rate=embedding_lr,
        deep_learning_rate=deep_lr,
        lr_schedule_horizon_epochs=40,
        seed=42,
        representation=G3Representation(
            history_representation=(
                "id_content_zero_id" if row_id.endswith(":02") else "id_content"
            ),
            catalog_representation="learned_id",
            history_hidden_dim=hidden,
        ),
        feature_data_path=feature_data_path,
    )


def write_job_contract(
    compiled: CompiledControlJob, ledger_path: Path, logs_root: Path
) -> Path:
    ledger = load_rq2_unexpected_diagnostic_ledger(ledger_path, root=PROJECT_ROOT)
    verified = decode_control_job(encode_control_job(ledger, compiled.row_id), ledger)
    if verified != compiled:
        raise ValueError(
            "compiled RQ2 unexpected diagnostic job differs from its ledger row"
        )
    return persist_job_contract(
        compiled=compiled,
        ledger_path=ledger_path,
        ledger_sha256=ledger.sha256,
        logs_root=logs_root,
        filename="g3_rq2_unexpected_diagnostic_job.json",
    )


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("ledger", type=Path)
    parser.add_argument(
        "--state-dir",
        type=Path,
        default=PROJECT_ROOT / "generated/training-queue-service",
    )
    parser.add_argument("--dry-run", action="store_true")
    arguments = parser.parse_args()
    print(
        submit_rq2_unexpected_diagnostic_jobs(
            ledger_path=arguments.ledger,
            state_dir=arguments.state_dir,
            dry_run=arguments.dry_run,
        )
    )


if __name__ == "__main__":
    main()
