...
def compile_queue_surface(
    *,
    ...
    expected = compile_rq4_single_metadata_width256_ledger(PROJECT_ROOT)
    if ledger != expected or len(ledger.rows) != 6:
        raise ValueError("RQ4 width-256 queue received another ledger")
    return compile_queue_commands(
        ...
        runner_script=Path(__file__).with_name(
            "run_rq4_single_metadata_width256.py"
        ),
...
def submit_jobs(
    *,
    ...
    ledger = load_rq4_single_metadata_width256_ledger(
        ledger_path,
        root=PROJECT_ROOT,
    ...
    runner = Path(__file__).with_name("run_rq4_single_metadata_width256.py")
...
def compiled_job_from_environment() -> tuple[
    CompiledControlJob, Rq4SingleMetadataWidth256Ledger, Path, Path
]:
    ...
    if decode_control_job(encode_control_job(ledger, compiled.row_id), ledger) != compiled:
        raise ValueError("compiled RQ4 width-256 job differs from immutable ledger")
...
def build_training_experiment(
    ...
    *,
    ...
    if not isinstance(representation, dict) or not isinstance(training, dict):
        raise ValueError("compiled RQ4 width-256 job is invalid")
    if (
        job.get("family_id") not in {"rq4_artist", "rq4_album"}
        or job.get("phase") != "rq4_single_metadata_width256_boundary_extension"
        or job.get("stage") != "rq4_single_metadata_width256_horizon25"
        or representation.get("metadata_dim") != 256
        or representation.get("metadata") not in (["artist"], ["album"])
        or training.get("horizon_epochs") != 25
        or training.get("batch_size") != 512
        or training.get("seed") != 42
    ):
        raise ValueError("RQ4 width-256 coordinate changed")
