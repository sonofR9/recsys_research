...
def compile_rq4_capacity_extension_queue_commands(
    *,
    ledger_path: Path,
    ledger: Rq4CapacityExtensionLedger,
    state_dir: Path,
    batch_id: str = "DRY_RUN_BATCH",
) -> list[list[str]]:
    expected = compile_rq4_capacity_extension_ledger(PROJECT_ROOT)
    if ledger != expected:
        raise ValueError("RQ4 width-128 queue received another ledger")
    return compile_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
        runner_script=Path(__file__).with_name("run_rq4_capacity_extension.py"),
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
        ledger_validator=lambda document: validate_rq4_capacity_extension_ledger_document(
            document,
            root=PROJECT_ROOT,
...
def submit_rq4_capacity_extension_jobs(
    *,
    ledger_path: Path,
    state_dir: Path,
    ...
    ledger = load_rq4_capacity_extension_ledger(
    ...
    if dry_run:
        return "\n".join(
            shlex.join(command)
            for command in compile_rq4_capacity_extension_queue_commands(
                ledger_path=ledger_path, ledger=ledger, state_dir=state_dir
            )
        )
    runner = Path(__file__).with_name("run_rq4_capacity_extension.py")
    with ledger_submission_lock(state_dir=state_dir, ledger_sha256=ledger.sha256):
        ...
        if existing is not None:
            return existing
        commands = compile_rq4_capacity_extension_queue_commands(
            ledger_path=ledger_path, ledger=ledger, state_dir=state_dir
        )
        subprocess.run(commands[0], cwd=PROJECT_ROOT, check=True, capture_output=True)
        created = subprocess.run(
            commands[1], cwd=PROJECT_ROOT, check=True, capture_output=True, text=True
        )
...
def compiled_job_from_environment() -> tuple[
    CompiledControlJob, Rq4CapacityExtensionLedger, Path, Path
]:
    ...
    initial = load_rq4_initial_ledger(
        PROJECT_ROOT / ledger.initial_ledger.path,
        root=PROJECT_ROOT,
    ...
    expected_compiled = decode_control_job(
        encode_control_job(ledger, compiled.row_id), ledger
    )
    if compiled != expected_compiled:
        raise ValueError("compiled RQ4 width-128 job differs from immutable ledger")
...
def build_training_experiment(
    ...
    *,
    ...
    if not isinstance(representation, dict) or not isinstance(training, dict):
        raise ValueError("compiled RQ4 width-128 job is invalid")
    if (
        job.get("phase") != "rq4_metadata_capacity_boundary_extension"
        or job.get("stage") != "rq4_metadata_capacity_width128_post_initial"
        or job.get("role") != "metadata_capacity_boundary_extension"
        or job.get("family_id") not in RQ4_METADATA_FAMILIES
        or representation.get("history") != "selected_rq2_content_concat"
        or representation.get("metadata_dim") != 128
        ...
        raise ValueError("RQ4 width-128 job violates the frozen contract")
...
def write_job_contract(
    compiled: CompiledControlJob, ledger: Rq4CapacityExtensionLedger,
    ledger_path: Path, logs_root: Path,
) -> Path:
    expected = decode_control_job(encode_control_job(ledger, compiled.row_id), ledger)
    if compiled != expected:
        raise ValueError("compiled RQ4 width-128 job differs from immutable ledger")
...
def main() -> None:
    ...
    print(submit_rq4_capacity_extension_jobs(
