...
from experiments.g3_pretrained_item_embeddings.protocol.rq5_global_boundary_ledger import (
    ...
    compile_rq5_global_boundary_ledger,
    load_rq5_global_boundary_ledger,
    persist_rq5_global_boundary_ledger,
    verify_rq5_global_boundary_inputs,
)


RQ5_GLOBAL_BOUNDARY_LEDGER_LOGICAL_SHA256 = (
    "a0c401cd6ba58324813d51fbcc749c64519b58401cf096c77dff9e1b82c91f65"
)
JOB_ENVIRONMENT = "G3_RQ5_GLOBAL_BOUNDARY_JOB_B64"
LEDGER_ENVIRONMENT = "G3_RQ5_GLOBAL_BOUNDARY_LEDGER_PATH"


def preview_rq5_global_boundary_ledger(
    *, root: Path = PROJECT_ROOT
) -> Rq5GlobalBoundaryLedger:
    ledger = compile_rq5_global_boundary_ledger(root=root)
    if ledger.sha256 != RQ5_GLOBAL_BOUNDARY_LEDGER_LOGICAL_SHA256:
        raise ValueError("RQ5 global boundary preview differs from approved SHA")
    return ledger


def materialize_rq5_global_boundary_ledger(
    path: Path | None = None, *, root: Path = PROJECT_ROOT
) -> Path:
    ledger = preview_rq5_global_boundary_ledger(root=root)
    destination = root / RQ5_GLOBAL_BOUNDARY_LEDGER_PATH if path is None else path
    return persist_rq5_global_boundary_ledger(destination, ledger, root=root)


def compile_rq5_global_boundary_queue_commands(
    *,
    ledger_path: Path,
    ...
    if len(ledger.rows) != 3 or len(ledger.physical_rows) != 3:
        raise ValueError("RQ5 global boundary queue requires exactly three jobs")
    commands = compile_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
        runner_script=Path(__file__).with_name("run_rq5_global_boundary.py"),
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
        ledger_validator=lambda document: _validate_document(document, ledger),
    )
    if len(commands) != 6:
        raise ValueError("RQ5 global boundary queue command count changed")
    return commands


def submit_rq5_global_boundary_jobs(
    *,
    ledger_path: Path,
    state_dir: Path,
    ...
    expected = preview_rq5_global_boundary_ledger(root=root)
    if expected.sha256 != expected_ledger_sha256:
        raise ValueError("RQ5 external global boundary SHA differs from preview")
    ...
    verify_rq5_global_boundary_inputs(root, ledger)
    if dry_run:
        return "\n".join(
            shlex.join(command)
            for command in compile_rq5_global_boundary_queue_commands(
...
def _submit(
    *,
    ...
    if existing is not None:
        return existing
    initial = compile_rq5_global_boundary_queue_commands(
        ledger_path=ledger_path, ledger=ledger, state_dir=state_dir
    )
    ...
    commands = compile_rq5_global_boundary_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
...
def compiled_rq5_global_boundary_job_from_environment(
    *, root: Path = PROJECT_ROOT
) -> tuple[CompiledControlJob, Rq5GlobalBoundaryLedger, Path, Path]:
    ...
    ledger = load_rq5_global_boundary_ledger(
        ledger_path,
        root=root,
        expected_ledger_sha256=RQ5_GLOBAL_BOUNDARY_LEDGER_LOGICAL_SHA256,
    )
...
def build_rq5_global_boundary_training_experiment(
    ...
    *,
    ...
    if expected != compiled:
        raise ValueError("compiled RQ5 global boundary job differs from ledger")
    row = next((value for value in ledger.rows if value.id == compiled.row_id), None)
    if row is None:
        raise ValueError("RQ5 global boundary row is not launchable")
...
def write_rq5_global_boundary_job_contract(
    compiled: CompiledControlJob,
    ledger: Rq5GlobalBoundaryLedger,
    ledger_path: Path,
    logs_root: Path,
) -> Path:
    loaded = load_rq5_global_boundary_ledger(
        ledger_path,
        root=PROJECT_ROOT,
...
def _validate_document(
    document: dict[str, object], ledger: Rq5GlobalBoundaryLedger
) -> None:
    if document != ledger.to_dict():
        raise ValueError("RQ5 global boundary document differs from approved ledger")
...
def main() -> None:
    ...
    print(
        submit_rq5_global_boundary_jobs(
