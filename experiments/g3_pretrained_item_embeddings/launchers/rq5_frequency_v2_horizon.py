...
from experiments.g3_pretrained_item_embeddings.protocol.rq5_frequency_v2_horizon_ledger import (
    ...
    compile_rq5_frequency_v2_horizon_ledger,
    load_rq5_frequency_v2_horizon_ledger,
    persist_rq5_frequency_v2_horizon_ledger,
...
JOB_ENVIRONMENT = "G3_RQ5_FREQUENCY_V2_HORIZON_JOB_B64"
LEDGER_ENVIRONMENT = "G3_RQ5_FREQUENCY_V2_HORIZON_LEDGER_PATH"


def preview_rq5_frequency_v2_horizon_ledger(
    *, root: Path = PROJECT_ROOT
) -> Rq5FrequencyV2HorizonLedger:
    ledger = compile_rq5_frequency_v2_horizon_ledger(root=root)
    if ledger.sha256 != RQ5_FREQUENCY_V2_HORIZON_LEDGER_LOGICAL_SHA256:
        raise ValueError("RQ5 frequency v2 horizon preview differs from approved SHA")
    return ledger


def materialize_rq5_frequency_v2_horizon_ledger(
    path: Path | None = None, *, root: Path = PROJECT_ROOT
) -> Path:
    ledger = preview_rq5_frequency_v2_horizon_ledger(root=root)
    destination = (
        root / RQ5_FREQUENCY_V2_HORIZON_LEDGER_PATH if path is None else path
    )
    return persist_rq5_frequency_v2_horizon_ledger(destination, ledger, root=root)


def compile_rq5_frequency_v2_horizon_queue_commands(
    *,
    ledger_path: Path,
    ledger: Rq5FrequencyV2HorizonLedger,
    state_dir: Path,
    batch_id: str = "DRY_RUN_BATCH",
) -> list[list[str]]:
    if len(ledger.rows) != 3 or len(ledger.physical_rows) != 3:
        raise ValueError("RQ5 frequency v2 horizon queue requires exactly three jobs")
    commands = compile_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
        runner_script=Path(__file__).with_name("run_rq5_frequency_v2_horizon.py"),
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
        ledger_validator=lambda document: _validate_document(document, ledger),
    )
    if len(commands) != 6:
        raise ValueError("RQ5 frequency v2 horizon queue command count changed")
    return commands


def submit_rq5_frequency_v2_horizon_jobs(
    *,
    ledger_path: Path,
    state_dir: Path,
    ...
    expected = preview_rq5_frequency_v2_horizon_ledger(root=root)
    if expected.sha256 != expected_ledger_sha256:
        raise ValueError("RQ5 external frequency v2 horizon SHA differs from preview")
    ...
    verify_rq5_frequency_v2_horizon_inputs(root, ledger)
    if dry_run:
        return "\n".join(
            shlex.join(command)
            for command in compile_rq5_frequency_v2_horizon_queue_commands(
...
def _submit(
    *,
    ...
    runner = Path(__file__).with_name("run_rq5_frequency_v2_horizon.py")
    ...
    if existing is not None:
        return existing
    initial = compile_rq5_frequency_v2_horizon_queue_commands(
        ledger_path=ledger_path, ledger=ledger, state_dir=state_dir
    )
    ...
    commands = compile_rq5_frequency_v2_horizon_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
...
def compiled_rq5_frequency_v2_horizon_job_from_environment(
    *, root: Path = PROJECT_ROOT
) -> tuple[CompiledControlJob, Rq5FrequencyV2HorizonLedger, Path, Path]:
    ...
    feature = verify_rq5_frequency_v2_horizon_inputs(root, ledger)
    return compiled, ledger, path, feature


def build_rq5_frequency_v2_horizon_training_experiment(
    ...
    *,
    ...
    if expected != compiled:
        raise ValueError("compiled RQ5 frequency v2 horizon job differs from ledger")
    row = next((row for row in ledger.rows if row.id == compiled.row_id), None)
    if row is None:
        raise ValueError("RQ5 frequency v2 horizon row is not launchable")
...
def write_rq5_frequency_v2_horizon_job_contract(
    compiled: CompiledControlJob,
    ledger: Rq5FrequencyV2HorizonLedger,
    ledger_path: Path,
    logs_root: Path,
) -> Path:
    loaded = load_rq5_frequency_v2_horizon_ledger(
        ledger_path,
        root=PROJECT_ROOT,
    ...
    return persist_job_contract(
        ...
        logs_root=logs_root,
        filename="g3_rq5_frequency_v2_horizon_job.json",
    )


def _validate_document(
    document: dict[str, object], ledger: Rq5FrequencyV2HorizonLedger
) -> None:
    if document != ledger.to_dict():
        raise ValueError("RQ5 frequency v2 horizon document differs from approved ledger")
...
def main() -> None:
    ...
    print(
        submit_rq5_frequency_v2_horizon_jobs(
