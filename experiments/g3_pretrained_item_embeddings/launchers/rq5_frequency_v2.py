...
from experiments.g3_pretrained_item_embeddings.protocol.rq5_frequency_v2_ledger import (
    ...
    compile_rq5_frequency_v2_ledger,
    load_rq5_frequency_v2_ledger,
    persist_rq5_frequency_v2_ledger,
    verify_rq5_frequency_v2_inputs,
)


RQ5_FREQUENCY_V2_LEDGER_LOGICAL_SHA256 = (
    "c7745c8ee5b2683c9e4001924926d1a92224a220b87269a36130224c8cfde8f4"
)
JOB_ENVIRONMENT = "G3_RQ5_FREQUENCY_V2_JOB_B64"
LEDGER_ENVIRONMENT = "G3_RQ5_FREQUENCY_V2_LEDGER_PATH"


def preview_rq5_frequency_v2_ledger(
    *, root: Path = PROJECT_ROOT
) -> Rq5FrequencyV2Ledger:
    ledger = compile_rq5_frequency_v2_ledger(root=root)
    if ledger.sha256 != RQ5_FREQUENCY_V2_LEDGER_LOGICAL_SHA256:
        raise ValueError("RQ5 frequency v2 preview differs from approved SHA")
    return ledger


def materialize_rq5_frequency_v2_ledger(
    path: Path | None = None, *, root: Path = PROJECT_ROOT
) -> Path:
    ledger = preview_rq5_frequency_v2_ledger(root=root)
    destination = root / RQ5_FREQUENCY_V2_LEDGER_PATH if path is None else path
    return persist_rq5_frequency_v2_ledger(destination, ledger, root=root)


def compile_rq5_frequency_v2_queue_commands(
    *,
    ledger_path: Path,
    ...
    if len(ledger.rows) != 9 or len(ledger.physical_rows) != 9:
        raise ValueError("RQ5 frequency v2 queue requires exactly nine jobs")
    commands = compile_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
        runner_script=Path(__file__).with_name("run_rq5_frequency_v2.py"),
        job_environment=JOB_ENVIRONMENT,
        ledger_environment=LEDGER_ENVIRONMENT,
        ledger_validator=lambda document: _validate_document(document, ledger),
    )
    if len(commands) != 12:
        raise ValueError("RQ5 frequency v2 queue command count changed")
    return commands


def submit_rq5_frequency_v2_jobs(
    *,
    ledger_path: Path,
    state_dir: Path,
    ...
    expected = preview_rq5_frequency_v2_ledger(root=root)
    if expected.sha256 != expected_ledger_sha256:
        raise ValueError("RQ5 external frequency v2 SHA differs from preview")
    ...
    verify_rq5_frequency_v2_inputs(root, ledger)
    if dry_run:
        return "\n".join(
            shlex.join(command)
            for command in compile_rq5_frequency_v2_queue_commands(
...
def _submit(
    *,
    ...
    if existing is not None:
        return existing
    initial = compile_rq5_frequency_v2_queue_commands(
        ledger_path=ledger_path, ledger=ledger, state_dir=state_dir
    )
    ...
    commands = compile_rq5_frequency_v2_queue_commands(
        ledger_path=ledger_path,
        ledger=ledger,
        state_dir=state_dir,
        batch_id=batch_id,
...
def compiled_rq5_frequency_v2_job_from_environment(
    *, root: Path = PROJECT_ROOT
) -> tuple[CompiledControlJob, Rq5FrequencyV2Ledger, Path, Path]:
    ...
    ledger = load_rq5_frequency_v2_ledger(
        ledger_path,
        root=root,
        expected_ledger_sha256=RQ5_FREQUENCY_V2_LEDGER_LOGICAL_SHA256,
    )
...
def build_rq5_frequency_v2_training_experiment(
    ...
    *,
    ...
    if expected != compiled:
        raise ValueError("compiled RQ5 frequency v2 job differs from ledger")
    row = next((value for value in ledger.rows if value.id == compiled.row_id), None)
    if row is None:
        raise ValueError("RQ5 frequency v2 row is not launchable")
...
def write_rq5_frequency_v2_job_contract(
    compiled: CompiledControlJob,
    ledger: Rq5FrequencyV2Ledger,
    ledger_path: Path,
    logs_root: Path,
) -> Path:
    loaded = load_rq5_frequency_v2_ledger(
        ledger_path,
        root=PROJECT_ROOT,
    ...
    return persist_job_contract(
        ...
        logs_root=logs_root,
        filename="g3_rq5_frequency_v2_job.json",
    )


def _validate_document(
    document: dict[str, object], ledger: Rq5FrequencyV2Ledger
) -> None:
    if document != ledger.to_dict():
        raise ValueError("RQ5 frequency v2 document differs from approved ledger")
