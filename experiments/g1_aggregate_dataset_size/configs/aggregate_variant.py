...
if os.environ.get("G1_DATASET_SIZE", "50m") != "50m":
    raise ValueError("native-50M aggregate runs cannot override G1_DATASET_SIZE")
if "G1_MAX_USERS" in os.environ:
    raise ValueError("native-50M aggregate runs require the full user population")
os.environ["G1_DATASET_SIZE"] = "50m"
os.environ["G1_MAX_EPOCHS"] = str(max(20, candidate.num_epochs))
...
def _with_member(
    experiment: GenerationExperiment, member: Member, candidate: AggregateCandidate
...
if candidate.family == "bridge":
    ...
    treatment = _with_member(treatment, candidate.member, candidate)
    ...
    for member in (
        ...
        treatment = _with_member(treatment, member, candidate)
    treatment = replace(
        treatment,
