...
if [ "$#" -ne 1 ]; then
    echo "usage: $0 REMEDIATION_LEDGER.json" >&2
    exit 2
fi

remediation_ledger=$1
project_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)
compiled_rows=$(
    python -m experiments.g6_rqkmeans_history.launchers.remediation_manifest \
        "$remediation_ledger"
)
rows=()
...
if [ "${G6_RQ0_REMEDIATION_QUEUE_DRY_RUN:-0}" = "1" ]; then
    for row in "${rows[@]}"; do
...
TRAINING_QUEUE_SCRIPT=$project_root/experiments/g6_rqkmeans_history/launchers/run_remediation.py
...
for row in "${rows[@]}"; do
    ...
    enqueue "$run_name" "G6_RQ0_REMEDIATION_JOB_B64=$payload" "WANDB_MODE=offline"
done
drain
